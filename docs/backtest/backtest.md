...
- **初始状态设置**：
  - 持仓股数 = 0（空仓启动）
  - 可用资金 = 总资金（`base_position_amount + grid_trading_amount`）
  - 当前基准价 = **网格价格上限**（`price_range.upper`）
  - 当前买入点 = 基准价 - 步长
  - 当前卖出点 = 基准价 + 步长
  - 价格下限 = `price_range.lower`
  - 价格上限 = `price_range.upper`
...
- **交易数量和价格**：
  - 交易数量 = `(1 + 倍数) × 单笔数量`
  - 买入价格（等差）= `下一买点 - 倍数 × 步长`
  - 买入价格（等比）= `下一买点 / (1 + 步长比例)^倍数`
  - 卖出价格（等差）= `下一卖点 + 倍数 × 步长`
  - 卖出价格（等比）= `下一卖点 × (1 + 步长比例)^倍数`
...
- **交易示例**：
  ...
  K线(最低/最高) | 触发判断        | 倍数计算                    | 倍数 | 交易价格计算 | 交易动作
  -------------|---------------|---------------------------|-----|------------|-------------
  3.785/3.815  | 未触发         | -                         | -   | - | 不交易
  3.770/3.800  | 低<=3.770触发  | |3.770-3.770|/0.030=0    | 0   | 3.770-0×0.030=3.770 | 买入100股，价格3.770
  3.740/3.760  | 低<=3.770触发  | |3.770-3.740|/0.030=1    | 1   | 3.770-1×0.030=3.740 | 买入200股，价格3.740
  3.680/3.710  | 低<=3.740触发  | |3.740-3.680|/0.030=2    | 2   | 3.740-2×0.030=3.680 | 买入300股，价格3.680
  3.690/3.720  | 高>=3.710触发  | |3.720-3.710|/0.030=0.3  | 0   | 3.710+0×0.030=3.710 | 卖出100股，价格3.710
  3.700/3.770  | 高>=3.720触发  | |3.770-3.720|/0.030=1.6  | 1   | 3.720+1×0.030=3.750 | 卖出200股，价格3.750
  ```
  
  **说明**：
  - 交易价格 = 下一网格点 ± 倍数 × 步长（等差）或 下一网格点 × (1±步长比例)^倍数（等比）
  - 每次交易后基准价更新为实际成交价
  - 下一买卖点根据新基准价重新计算
  - 倍数向下取整，例如1.6向下取整为1
...
- **成交价格**：
  - **买入成交价**（等差）= `下一买点 - 倍数 × 步长`
  - **买入成交价**（等比）= `下一买点 / (1 + 步长比例)^倍数`
  - **卖出成交价**（等差）= `下一卖点 + 倍数 × 步长`
  - **卖出成交价**（等比）= `下一卖点 × (1 + 步长比例)^倍数`
  - 理由：按实际跨越的网格数计算成交价
...
| 决策项 | 确认方案 |
|--------|---------|
| 📊 初始建仓 | **空仓启动**，以网格上限为基准价，全部资金作为现金 |
| 💰 成交价格 | **买入=下一买点-倍数×步长，卖出=下一卖点+倍数×步长**（等差）或按比例计算（等比） |
